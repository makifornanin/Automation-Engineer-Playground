...
- [ ] Two tables created: `dlq_events` and `execution_logs` (SQL for both is below)
...
# Step 8b - Log the Failure

A DLQ tells you *that* something failed. It does not tell you the story of the
run that produced it — and when you are staring at a queued event tomorrow
morning, the story is what you want.

Add a second **Supabase** node connected from `Build DLQ Record` — alongside
`Save to DLQ`, not after it. Rename it:

```text
Log DLQ Failure
```

`Build DLQ Record` now feeds two branches: one saves the event so it can be
replayed, the other records what happened. They do different jobs and both end
at the response.
...
Two of those values come from n8n itself rather than your data. `$workflow.name`
and `$execution.id` identify *which run* wrote the record, so you can jump
straight from a log row back to the execution in n8n.

The table lives in the observability standard, not this lab, because Lab 08 is
not the only thing that will write to it. Create it now if you have not already:
...
Full field reference: [`docs/observability.md`](../../docs/observability.md).

---

# Step 9 - Return Queued for Recovery

Both branches — `Save to DLQ` and `Log DLQ Failure` — end here. Add a Respond
to Webhook node and connect both into it.
...
# Step 16b - Log the Recovery

Add a **Supabase** node after `Mark DLQ Recovered`. Rename it:
...
| `workflow_name` | `{{ $workflow.name }}` |
| `execution_id` | `{{ $execution.id }}` |
| `event_id` | `{{ $('Fetch Pending DLQ Event').item.json.event_id }}` |
| `status` | `recovered` |
| `stage` | `recovery_completed` |
| `error_message` | *leave this blank* |
| `retry_count` | `{{ $('Fetch Pending DLQ Event').item.json.retry_count }}` |
| `recovered` | `true` |

Notice where `event_id` comes from. `Mark DLQ Recovered` returns the *updated
row*, and the recovery run is a different execution from the one that failed —
so we reach back to `Fetch Pending DLQ Event` for the business identity.

That is the whole trick. The recovery ran hours later under a new
`execution_id`, but it carries the **same `event_id`** as the failure. Query
that one value and both rows come back together:

```sql
select logged_at, execution_id, status, stage, recovered
from execution_logs
where event_id = 'evt_dlq_001'
order by logged_at;
```

You should see two rows: a `failed` / `dlq_queued`, then a `recovered` /
`recovery_completed`. Two separate executions, one continuous story.

That is what observability buys you — and it is exactly the question a DLQ
leaves you asking.
