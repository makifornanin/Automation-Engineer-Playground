...
export const LAB_06_CHUNKS: readonly LessonChunk[] = [
  ...
  },
  {
    ...
    actions: [
      {
        text: "Add a Manual Trigger named Start Lab 06 and an Edit Fields node named Set Retry Config.",
        code: {
          language: "json",
          code: [
            "{",
      ...
      {
        text: "Add the Simulate API Request Code node from the lab's workflow export — it returns predictable 429, 503 and 200 responses so the lab does not depend on a genuinely unreliable service.",
      },
      {
        text: "Add an IF named Was Request Successful? checking response.status_code equals 200, and on FALSE a Code node named Classify Failure that marks the failure retryable when the status is 429, 500, 502, 503 or 504.",
  ...
  },
  {
    ...
    actions: [
      {
        text: "Add an IF named Should Retry? requiring both failure.retryable is true AND attempt is less than max_attempts.",
      },
      {
        text: "On TRUE add a Code node named Calculate Backoff that doubles the delay each attempt.",
        code: {
          language: "javascript",
          code: [
            "const backoff = $json.base_delay_seconds * 2 ** ($json.attempt - 1);",
            "const jitter = Math.random();",
            "return { json: { ...$json, retry: { delay_seconds: backoff + jitter } } };",
          ].join("\n"),
        },
      },
      {
        text: "Add a Wait node named Wait Before Retry using that delay, then a Code node named Increase Retry Counter that adds one to attempt.",
      },
      {
        text: "Connect Increase Retry Counter back into Simulate API Request, and put a Return Success node on the TRUE branch of Was Request Successful?",
